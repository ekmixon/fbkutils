...
#		print 'Line: ', line
            ...
            for kv in kvals:
                ...
                if key == field:
                    if val.find('/') > 0:
                        val = val.split('/')[0] if field == 'rtt' else val.split('/')[1]
                    if field == 'send':
                        p = val.find('bps')
                        if p > 0:
                            m = val[p-1]
...
if field == 'bytes_acked':
    field = 'acked_rate'
p = psPlot.PsPlot(f'{path}/{field}', '', '', 1)
...
for i, X in enumerate(XList):
    ...
    p.PlotData(
        ...
        '',
        (
            f'0.4 {p.SetColor(p.colors[(i + 1) % p.colorsN])}'
            + ' plotStraightLinesC'
        ),
    )
